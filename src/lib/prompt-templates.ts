...
const nichoHints: Record<string, string> = {
  Barbearia:
    "Foto de cortes reais, agendamento online, catálogo de serviços com preços, galeria estilo Instagram, seção de barbeiros.",
  Restaurante:
    "Cardápio digital categorizado, fotos apetitosas, reservas, mapa da localização, avaliações reais.",
  ...
  Advogado:
    "Áreas de atuação, biografia, conteúdo educativo, agendamento de consulta, casos de sucesso.",
...
export function generatePrompt(b: BriefingLike): string {
  ...
  parts.push("");
  parts.push(
    `Você é uma agência sênior de design e desenvolvimento. Crie um website institucional **premium, responsivo, rápido e otimizado para SEO local** para o cliente descrito abaixo.`,
  );
  parts.push("");
  ...
  parts.push(
    [
      ...
      line(
        ...
        b.style ||
          "Minimalista premium, tipografia moderna, muito espaço em branco, cantos 20px, sombras suaves.",
      ),
  ...
  const pages =
    (b.pages && b.pages.trim()) || "Home, Sobre, Serviços, Galeria, Depoimentos, Contacto";
  parts.push(
    `- ${pages
      .split(/[,\n]/)
      .map((p) => p.trim())
  ...
  parts.push(
    [
      b.whatsapp
        ? `- Botão flutuante de WhatsApp com link \`https://wa.me/${(b.whatsapp || "").replace(/\D/g, "")}?text=Olá, vim pelo site\`.`
  ...
  parts.push(
    "Gere o projeto completo, com todas as páginas indicadas, componentes reutilizáveis, sistema de design centralizado, dados de exemplo realistas do próprio cliente, e um README explicando como personalizar cores, textos e imagens.",
  );
